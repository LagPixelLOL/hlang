...
|---|---|---|---|---|
| 1 | JIT uses the task's hash sym table + parent tasks' tables; syms scope like environment vars | no (documented) | — | hosted hcc compiles one program per invocation; there are no tasks. `extern`/`import` resolve against the process/link instead (README limitation) |
...
| 4 | `extern` binds to an existing same-name sym; "also can be used to generate a fwd reference" | yes | `anti_c/anti_c_fwd_extern` (mutual recursion + fwd-referenced global defined later) | bare C-style prototypes are tolerated as the same thing (README) |
| 5 | `import` binds at `Load()` time; incomplete until defined | partial | accepted (prelude parse path) | there is no `Load()` in a hosted world: `import` behaves like `extern` (README) |
...
| 7 | `_import` — different-name bind at `Load()` time | partial | accepted | ≡ `_extern` in the hosted world (README) |
...
| 16 | `asm` label rows (export/local/IMPORT) | — | no (documented) | `errors/asm` | inline asm is out of scope entirely (README limitation) |
...
| 20 | `reg`/`noreg` override reg-var allocation; optional named reg | parsed | `cases/misc` | LLVM owns register allocation; annotations are accepted and ignored (README) |
| 21 | Local vars accessible in asm via `&i[RBP]`; globals via `[&glbl_var]` | no | — | asm out of scope (README) |
| 22 | Offspring tasks inherit syms | no | — | no tasks (README) |
...
| 27 | `OPTf_EXTERNS_TO_IMPORTS` treats externs as imports (JIT/AOT dual headers) | accepted (no-op) | `cases/misc` (`Option()` path) | unnecessary in hcc: `import` ≡ `extern` already (README) |
