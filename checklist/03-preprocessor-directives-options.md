...
| 4 | `#exe {}` executes at compile time, `StreamPrint()` inserts source; `#exe {Cd("DirName");;}` idiom | partial | `cases/exe` (consts, statements, `#define` injection), `examples/CompileTime.HC` (tables, functions, strings) | `StreamPrint` fully supported; `Cd()`/`StreamDir` are TempleOS-task facilities and are not provided (README limitation) |
...
| 11 | `#help_index`, `#help_file` (Help System) | accepted, ignored | `cases/preproc` | there is no Help System to feed (README) |
...
`Fs->last_cc` does not exist (README limitation).
...
| 17 | `__DIR__` (`StreamDir`) | yes | `cases/preproc` (asserts non-empty — value is invocation-path dependent) | builtin; standalone `StreamDir` is not provided (README) |
...
| 26 | `OPTf_NO_BUILTIN_CONST` (disable 10-byte float consts for `pi`, `log2_10`, `log10_2`, `loge_2`) | N/A | `cases/floats` (`pi` value) | the four consts exist as F64 prelude `#define`s; there are no 80-bit floats to disable (README limitation) |
